...
class SignInForm extends React.Component {
  ...
  renderTextField = ({ input, label, type, autoFocus }) => {
    return (
  ...
  render() {
    ...
    const errors =
      this.props.errors <= 0 ? null : renderErrors(this.props.errors);
    if (username != null) {
      usernameQRCode = username;
    }
    return (
      <form onSubmit={handleSubmit} className={classes.formLogin}>
        ...
        {username != null && (
          <Field
            name="username"
            type="username"
            component={props => <input type="hidden" value={username} />}
            classes={classes}
            value="ahmed"
          />
        )}
...
const validate = values => {
  const errors = {};
  if (usernameQRCode != null) {
    values.username = usernameQRCode;
  }
  if (!values.username) {
